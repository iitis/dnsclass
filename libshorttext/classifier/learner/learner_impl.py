...
# 		idx = v.index - 1
# 		val = v.value
# 		wB = m.get_weight(idx, 1)
# 		wM = m.get_weight(idx, 6)
# 		print "feature %d has value %g: browsing=%g, mail=%g" \
# 			% (idx, val, wB, wM) 
...
def predict(data_file_name, m, liblinear_opts=""):
	"""
	Return a 5-tuple: the predicted labels, the accuracy, the decision values, the
	true labels, and number of features in the test data file (obtained through the
	:class:`LearnerModel` *m*).
	...
	>>> predicted_label, accuracy, all_dec_values, label, feat_cnt = predict('svm_file', model)
	...
	for i in range(learner_prob.l):
		...
		# pjf
		j = 0
		while learner_prob.x[i][j].index != -1: j += 1
		fc += [(j-1)]
	...
	return py, acc, all_dec_values, ty, fc
